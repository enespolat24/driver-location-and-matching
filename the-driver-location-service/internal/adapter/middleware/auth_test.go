...
// TestAPIKeyAuthMiddleware_HealthEndpointSkipsAuth tests that health endpoint bypasses authentication
// Expected: Should allow request to proceed without API key for health endpoint
...
// TestAPIKeyAuthMiddleware_RootEndpointSkipsAuth tests that root endpoint bypasses authentication
// Expected: Should allow request to proceed without API key for root endpoint
func TestAPIKeyAuthMiddleware_RootEndpointSkipsAuth(t *testing.T) {
	e := echo.New()
...
// TestAPIKeyAuthMiddleware_SwaggerEndpointSkipsAuth tests that swagger endpoints bypass authentication
// Expected: Should allow request to proceed without API key for swagger endpoints
func TestAPIKeyAuthMiddleware_SwaggerEndpointSkipsAuth(t *testing.T) {
	e := echo.New()
	swaggerPaths := []string{
		"/swagger/index.html",
		"/swagger/doc.json",
	...
	for _, path := range swaggerPaths {
		...
		err := h(c)
		assert.NoError(t, err)
		assert.Equal(t, http.StatusOK, rec.Code, "Path %s should skip authentication", path)
		assert.Equal(t, "swagger", rec.Body.String())
	}
}
